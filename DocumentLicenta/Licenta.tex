...
\end{center}
\end{figure}
\par Arhitectura aleasă pentru autoencodere este cea descrisă în \emph{Ionescu et al.}  \cite{ionescu2019object} si constă intr-o arhitectură convoluţională rapidă, formată dintr-un encoder cu 3 blocuri convoluţional + max-pooling şi un decoder format din 3 blocuri upsampling + convoluţional. Fiecare autoencoder primeşte ca input date de dimensiune \(64 \times 64\) si creează după encoder un vector de caracteristici de dimensiune \(8 \times 8 \times 16\).
\par
Implementarea respectă arhitectura prezentată în figura ~\ref{fig:stage1_architecture} şi constă în următorii paşi: \\
\begin {itemize}
\item Pentru toate videourile de antrenare se execută detecţia de obiecte pentru fiecare cadru, extrăgând astfel toate obiectele din video. Pentru fiecare obiect este calculat gradientul, ce reflectă mişcarea acestuia. Gradientul este calculat dupa formula: \[\sqrt{G_{x}^2 + G_{y}^2}\] Unde \(G_{x}\), \(G_{y}\) sunt imagini ce în fiecare punct conţin derivata orizontală respectiv verticală a imaginii iniţiale, obţinută prin aplicarea unui kernel Sobel de \(5 \times 5 \).
...
\item Folosind cele 2 colecţii (de imagini, respectiv de grandienţi) este antrenat câte un autoencoder pentru fiecare colecţie. Antrenarea se realizează folosind optimizatorul Adam \cite{adam2017} şi funcţia loss ce constă în diferenţa medie pătratelor, dată de formula : 
...
\par
În etapa de antrenare a clasificatorilor finali.
